...
function memoize(func) {
  const cache = new Map();
  return (...args) => {
    const argsSt = JSON.stringify(args);
    if (cache.has(argsSt)) {
      return cache.get(argsSt);
    }
    const resut = func(...args);
    cache.set(argsSt, resut);
    return resut;
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    function writeParams() {
      ...
      const strInit = `${func.name}(${params})`;
      return strInit;
    }
    const strInit = `${writeParams()} starts`;
    logFunc(strInit);
    const result = func(...args);
    const strEnd = `${writeParams()} ends`;
    logFunc(strEnd);
    return result;
  };
}
